...
# create dictionary tracking number of dlinemen on a given play
number_dict = {0: [], 1: [], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[]}
current_play = '2630 10'
counter = 0
for index, row in data.iterrows():
    if current_play != row['PlayID']:
        number_dict[counter].append(current_play)
        counter = 0
        current_play = row['PlayID']
        if row['OnFieldPosition'] == 'DL':
            counter += 1
    else:
        if row['OnFieldPosition'] == 'DL':
            counter += 1
...
data = data[data['PlayID'].isin(number_dict[4])]
